...
for _ in range(NUM_ROWS):
    ...
    # --- Label (Zielvariable) ---
    # Regelbasierte Zuweisung, um "lernbare" Muster zu erstellen

    fraud_score = 0
    label = 0

    # Regel 1: Hohe Abweichung der Werbungskosten vom Durchschnitt der Berufsgruppe
    if abs(abweichung_werbungskosten_von_berufsgruppe) > 0.12:
        fraud_score += 2

    # Regel 2: Große Diskrepanz zwischen gemeldetem Einkommen und Lohnbescheid
    if abs(differenz_einkommen_lohnbescheid) > 400:
        fraud_score += 3

    # Regel 3: Extrem hohe Werbungskosten im Verhältnis zum Einkommen
    if ratio_werbungskosten_zu_einkommen > 0.35:
        fraud_score += 1

    # Regel 4: Extrem hoher Anstieg der Werbungskosten im Vergleich zum Vorjahr
    if veraenderung_werbungskosten_vj_prozent > 1.5: # +150%
        fraud_score += 2

    # Regel 5: Hohe Geschäftskosten (Reisen + Bewirtung) bei hohem Anstieg
    if (kosten_geschaeftsreisen + kosten_bewirtung > 10000) and veraenderung_werbungskosten_vj_prozent > 0.8:
        fraud_score += 3

    # Regel 6: Hohe angebliche Krankheitskosten (Außerg. Belastungen)
    if summe_ausserg_belastungen > 6000:
        fraud_score += 1

    # Regel 7: Abweichung bei Gewinnmarge (für Selbständige/Gewerbe)
    if hat_anlage_g and abs(abweichung_gewinnmarge_von_branche) > 0.08:
        fraud_score += 2

    # Setze Label basierend auf Score
    # Wir wollen immer noch ein unausgeglichenes Set, also muss der Schwellenwert relativ hoch sein.
    if fraud_score >= 4:
        label = 1

    # Einführung von Rauschen:
    # 1% Chance, dass ein "Not Fraud" (score < 4) fälschlicherweise als Fraud markiert wird (z.B. Fehler bei Dateneingabe)
    if label == 0 and random.random() < 0.01:
        label = 1
    # 5% Chance, dass ein "Fraud" (score >= 4) fälschlicherweise als "Not Fraud" durchgeht (z.B. geschickte Verschleierung)
    elif label == 1 and random.random() < 0.05:
        label = 0
...
# Zeige die Verteilung des Labels an
...
output_filename = 'fake_steuerdaten.csv'
